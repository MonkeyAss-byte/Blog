...
function setupCustomCursor() {
  ...
  const render = () => {
    // Position: high lerp keeps latency very low
    const lerp = 0.6;
    ...
    // Real mouse velocity = how far the physical mouse moved this frame
    const velX = mouseX - prevMouseX;
    const velY = mouseY - prevMouseY;
    prevMouseX = mouseX;
    prevMouseY = mouseY;

    // Compute target tilt from real velocity. 
    // divisor=8 → moving 8px/frame → full 45° tilt (normal fast movement is ~15-30px/frame)
    const maxTilt = 45;
    let targetTiltX = (velY / 8) * -maxTilt;
    let targetTiltY = (velX / 8) * maxTilt;
    targetTiltX = Math.max(-maxTilt, Math.min(maxTilt, targetTiltX));
    targetTiltY = Math.max(-maxTilt, Math.min(maxTilt, targetTiltY));

    // Spring-lerp the tilt so it hangs and bounces back (0.12 = slow spring)
    currentTiltX += (targetTiltX - currentTiltX) * 0.12;
    currentTiltY += (targetTiltY - currentTiltY) * 0.12;

    // Scale: squish when velocity is high, crush on click
    const speed = Math.min(Math.sqrt(velX * velX + velY * velY), 30);
    let targetScale = 1 - (speed / 30) * 0.18;
    if (isPressing) targetScale = Math.min(targetScale, 0.78);
    currentScale += (targetScale - currentScale) * 0.2;

    const activeCursor = document.getElementById("fluent-cursor");
    if (activeCursor) {
      activeCursor.style.transform =
        `translate3d(${cursorX}px, ${cursorY}px, 0) rotateZ(15deg) perspective(600px) rotateX(${currentTiltX}deg) rotateY(${currentTiltY}deg) scale(${currentScale})`;
    }
    ...
    const settled =
      ...
      Math.abs(velX) < 0.05 &&
      Math.abs(velY) < 0.05 &&
      Math.abs(currentTiltX) < 0.1 &&
      Math.abs(currentTiltY) < 0.1 &&
    ...
    if (settled) {
      ...
      if (activeCursor) {
        activeCursor.style.transform =
          `translate3d(${mouseX}px, ${mouseY}px, 0) rotateZ(15deg) perspective(600px) rotateX(0deg) rotateY(0deg) scale(1)`;
      }
    } else {
      rafId = requestAnimationFrame(render);
    }
